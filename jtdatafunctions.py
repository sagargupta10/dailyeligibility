...
def get_jtchequedata(custList, googlesecretkey_location, rs_user_id, rs_password):
    import jthelperfunctions as jthf
    import pandas as pd
    import re           
    ...
    chequeQuery = '''
    SELECT 
        date(last_updated_time) AS "date", 
        sender AS "bid", 
        amount AS "amount", 
        'Collected' AS "finalstatus", 
        'NA' AS "bouncereason", 
        0 AS "repldays"
    FROM 
        transaction 
    WHERE
        ...
        AND last_updated_time < CURRENT_DATE + 1 
        AND deleted_at IS NULL
    '''

    #Taking today's cheque data from Last-mile transactions table only for today. (Past data we will still refer from excel sheets)
    tempChequeUsageData=jthf.getDataFromRedshift(chequeQuery,rs_user_id,rs_password)

    tempChequeUsageData['amount'] = tempChequeUsageData['amount'].astype(float)
    tempChequeUsageData['repldays'] = tempChequeUsageData.repldays.apply(lambda x: jthf.ensureNum(x))
    tempChequeUsageData['date'] = pd.to_datetime(tempChequeUsageData.date)

    # print("")
    # print("---- NEW CHEQUE DATA FROM LM ----")
    # print("")

    # print(tempChequeUsageData.head())
    # print("")
    # print("---- LM data size ----")
    # print("")
    # print(tempChequeUsageData.shape)
    ...
    # print("")
    # print("---- EXCEL CHEQUE DATA ----")
    # print("")

    # print(chequeTransData.head())

    # print("")
    # print(chequeTransData.shape)

    # print("")
    # print("---- APPENDED DATA ----")
    # print("")

    # chequeTransData = chequeTransData.append(tempChequeUsageData)
    # print(chequeTransData.head())
...
def get_creditdata(custList,googlesecretkey_location,rs_user_id,rs_password):
    import jthelperfunctions as jthf
    import pandas as pd
    ...
    # print("")
    # print("------------------------- CREDIT TRANS DATA UNFILTERED ----------------------------")
    # print("")
    # print(creditTranData.head())

    # print("")
    # print(creditTranData.shape)    

    creditQuery='''
    SELECT 
        txn.sender AS "bid",
        crd.dpn_amount AS "amount",
        '' AS "status",
        '' AS "ever_bounced",
        0 AS "repayDays",
        0 AS "repayAttempts"
    FROM 
        transaction txn
        JOIN credit crd ON txn.transaction_id = crd.transaction_id
    WHERE
        txn.last_updated_time >= CURRENT_DATE
        AND txn.last_updated_time < CURRENT_DATE + 1 
        AND txn.deleted_at IS NULL
    '''

    # tempCreditUsageData=jthf.getDataFromRedshift(creditQuery,rs_user_id,rs_password)

    # tempCreditUsageData.rename(index=str,
    #                       columns={'repaydays':'repayDays',
    #                                'repayattempts':'repayAttempts'}, inplace=True)

    # print("")
    # print("------------------------- REDSHIFT REAL-TIME TRANS DATA ----------------------------")
    # print("")
    # print(tempCreditUsageData)

    # print("")
    # print(tempCreditUsageData.shape) 

    # creditTranData = creditTranData.append(tempCreditUsageData)

    # print("")
    # print("------------------------- MERGED CHEQUE DATA ----------------------------")
    # print("")
    # print(creditTranData.head())
...
#    cdView.columns = cdColNames
    ...
    scmViewCols = [
            ...
            'totalBouncedCheques',
            'totalBouncedChequeAmount',
            'totalChequesEverCount',
            'totalChequesEverAmount',
            'eligibleForCredit',
            'creditProvider',
            'currentCreditLimit',
            'totalCreditLimit',
            'creditTransactionLimit',
            'bouncedPDC',
            'outstandingCreditBouncedAmount',
            'outstandingPDC',
            'outstandingPDCAmount',
            'totalCreditBouncedPDC',
            'totalCreditBouncedPDCAmount',
            'creditEverUsedCount',
            'creditEverUsedAmount',
            'cheque_createdTime',
    ...
    scmView.loc[:,'outstandingBouncedCheques']=refreshedData.currBouncedCount
    scmView.loc[:,'outstandingBouncedChequesAmount']=refreshedData.currBouncedValue
    scmView.loc[:,'totalBouncedCheques']=refreshedData.everBouncedCount
    scmView.loc[:,'totalBouncedChequeAmount']=refreshedData.everBouncedValue
    scmView.loc[:,'totalChequesEverCount']=refreshedData.totalChequesEver
    scmView.loc[:,'totalChequesEverAmount']=refreshedData.totalChequesValue
    ...
    scmView.loc[:,'outstandingCreditBouncedAmount']=refreshedData.currCreditBouncedValue
    ...
    scmView.loc[:,'totalCreditBouncedPDC']=refreshedData.creditEverBouncedCount
    scmView.loc[:,'totalCreditBouncedPDCAmount']=refreshedData.creditEverBouncedValue
    scmView.loc[:,'creditEverUsedCount']=refreshedData.creditEverUseCount
    scmView.loc[:,'creditEverUsedAmount']=refreshedData.creditEverUseValue
...
def publishLimitsTest(refreshedData, callExceedLimits, googlesecretkey_location):
    ...
    #write permisison and send GS for exceed Calls
    #callExceedLimits
    tkrFileName="TEST_NEW4_ChequeCreditLimitCalls_"+todStr
    print("Writing ", tkrFileName, " to Googlesheets...")
    exceededGSheet = jthf.writeGsheet(callExceedLimits,'A1',tkrFileName,"Sheet1", googlesecretkey_location)
    #permission send
...
#    cdView.columns = cdColNames
    ...
    #set filename
    #Check_Credit_Reference_Nov_11_2017
    tkrFileName="TEST_NEW4_Check_Credit_Reference_"+todStr
    print("Writing ", tkrFileName, " to Googlesheets...")
    #write CD view to GS
    cdGSheet = jthf.writeGsheet(cdView,'A1',tkrFileName,"Sheet1", googlesecretkey_location)
    #permission CD sheet and send
...
#%%    #write SCM view to GS
    tkrFileName="TEST_NEW4_Check_Credit_Reference_SCM_"+todStr
    print("Writing ", tkrFileName, " to Googlesheets...")
    scmGSheet = jthf.writeGsheet(scmView,'A1',tkrFileName,"Sheet1", googlesecretkey_location)
    #permission send
